...
#LSTM: 16 hidden units, bidirectional,  3 layers of LSTM
#Linear Layer: Output layer of 2 output units
...
class LSTMnetwork(nn.Module):
	def __init__(self, dim_input, init_alpha = 1.0, dropout =0.1, num_layers=1):
		...
		self.rnn1 = nn.LSTM(input_size=dim_input, hidden_size=8, batch_first=True, num_layers = num_layers, bidirectional=True, dropout = dropout)
		self.linear2 = nn.Linear(in_features = 16, out_features =2)
		self.init_alpha = init_alpha
		self.batchnorm1 = torch.nn.BatchNorm1d(num_features = 125)
		self.batchnorm2 = torch.nn.BatchNorm1d(num_features = 16)
...
model = LSTMnetwork(num_features, init_alpha = init_alpha, dropout = dropout, num_layers=num_layers)
