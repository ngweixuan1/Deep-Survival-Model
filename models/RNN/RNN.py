...
#RNN: 32 hidden units,  bidirectional, dropout 0.3, 1 layer
#Linear Layer:  hidden layer of 16 and 2 output units 
...
class featureslabel(Dataset):
	...
	def __init__(self, seqs, labels, num_periods = 1024):
		...
		for i, id in enumerate(seqs_id):
				...
				# For each id, place them into the correct tensor  format
...
class RNNnetwork(nn.Module):
	def __init__(self, dim_input, init_alpha = 1.0, dropout =0.1, num_layers=1):
		...
		self.rnn1 = nn.RNN(input_size=dim_input, hidden_size=128, batch_first=True, num_layers = num_layers, bidirectional=True, dropout = dropout)
...
model = RNNnetwork(num_features, init_alpha = init_alpha, dropout = dropout, num_layers=num_layers)
