...
#Transformers: 3 hidden layers with ReLU activation function, 8 heads
#Linear layer:  Hidden layer of 32 and 2 output units
...
num_periods = train_seqs.period.max()+1
...
class TransformerNetwork(nn.Module):
	def __init__(self, dim_input, init_alpha = 1.0, dropout =0.1, nhead=1):
		super(TransformerNetwork, self).__init__()
		self.encoderlayer = nn.TransformerEncoderLayer(d_model = dim_input, nhead = nhead, batch_first=True, activation ='relu', dropout = dropout)
		self.encoder = nn.TransformerEncoder(self.encoderlayer, num_layers = 3)
		self.linear2 = nn.Linear(in_features = dim_input, out_features =32)
		self.linear3 = nn.Linear(in_features = 32, out_features =2)
		self.relu = nn.ReLU()
		self.init_alpha = init_alpha

	def activation_function(self, output): # custom activation function to get alpha and beta 
...
# intilization model network
model = TransformerNetwork(num_features, init_alpha = init_alpha, dropout = dropout, nhead=nhead)
